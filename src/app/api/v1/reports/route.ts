...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await get(alunoId);
    ...
    const recentActivity = report.recentActivity.length > 0
      ? report.recentActivity.map((a: any) => {
          ...
          return {
            ...
            text: a.type === "session_complete"
              ? `Sessão concluída - ${a.description}`
              : `Sessão - ${a.description}`,
            date: dateLabel,
          };
        })
      : [{ icon: "info", text: "Nenhuma sessão realizada ainda. Explore mentores para começar!", date: "" }];
